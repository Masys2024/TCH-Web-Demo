...
export default function AdminPage() {
  const data = STAFFS?.filter((staff) => staff.Role === "Admin").map(
    (staff) => ({
  ...
  const columns = [
    ...
    {
      accessorKey: "Owner_Rights",
      header: "Owner Rights",
      cell: ({ row }) => (
        <span>{`${row?.original?.Owner_Rights ? "Yes" : "No"}`}</span>
      ),
    },
    {
      accessorKey: "Biometric_Rights",
      header: "Biometric Rights",
      cell: ({ row }) => (
        <span>{`${row?.original?.Biometric_Rights ? "Yes" : "No"}`}</span>
      ),
    },
    {
      accessorKey: "actions",
  ...
  return (
    <section className="p-6 max-w-7xl mx-auto">
      <div className="w-full pb-8 flex items-center justify-between">
        ...
        <Link href={INTERNAL_LINKS.ADD_ADMIN}>
          <Button>
